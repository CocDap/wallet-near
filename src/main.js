...
import checkCredentials from "near-cli/utils/check-credentials";
import connect from "near-cli/utils/connect";


import inspectResponse from "near-cli/utils/inspect-response";
...
async function sendMoney(){
  console.log(window.nearConfig.networkId);
  console.log(window.accountId);
  console.log(window.nearConfig.walletUrl);
  console.log(keyStore)
  await checkCredentials(window.accountId,window.nearConfig.networkId,keyStore);
  const account = await window.near.account(window.accountId);
  console.log(account);
  const result = await account.sendMoney("dungho.testnet", utils.format.parseNearAmount("1"));
  console.log(result);
  inspectResponse.prettyPrintResponse(result);
